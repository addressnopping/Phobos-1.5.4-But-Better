...
public class HUD extends Module
{
    ...
    public void onRender2D(final Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case PHOBOSBB: {
                this.renderer.drawString("Phobos But Better!" + (this.modeVer.getValue() ? " v1.5.4" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? ((int)this.colorMap.get(2)) : this.color, true);
                break;
            }
            case PHOBOSB: {
                this.renderer.drawString("PhobosBetter" + (this.modeVer.getValue() ? " v1.5.4" : ""), 2.0f, 2.0f, (this.rolling.getValue() && this.rainbow.getValue()) ? ((int)this.colorMap.get(2)) : this.color, true);
                break;
            }
        }
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to PhobosBetter " + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
